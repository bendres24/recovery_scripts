...
root = tk.Tk()
root.withdraw()
folder_path = filedialog.askdirectory(title="Select folder of 8-bit multispectral images")
if not folder_path:
...
# -------------------- ROI Selection --------------------
p_low, p_high = np.percentile(images[0], (2, 98))
img_clip = np.clip(images[0], p_low, p_high)
disp_norm = cv2.normalize(img_clip, None, 0, 255, cv2.NORM_MINMAX)
...
for method, (stack, eig) in results.items():
    out_dir = os.path.join(folder_path, f"{method}_Results_8bit")
...
plt.title("Comparison of Eigenvalue / Variance Curves (8-bit Images)")
...
plot_path = os.path.join(folder_path, "Eigenvalue_Comparison_8bit.png")
