...
root = tk.Tk()
root.withdraw()
folder_path = filedialog.askdirectory(title="Select folder of 16-bit RGB TIFF images")
if not folder_path:
...
for f in files:
    ...
    h, w, _ = img.shape
    image_stack = img.transpose(2, 0, 1)
    ...
    for method, (stack, eig) in results.items():
        out_dir = os.path.join(folder_path, f"{method}_Results_16bit_RGB")
        ...
        for i in range(stack.shape[0]):
            ...
            out_path = os.path.join(out_dir, f"{os.path.splitext(f)[0]}_{method}_Comp{i+1}.tif")
            tiff.imwrite(out_path, scaled.astype(np.uint16), photometric='minisblack', compression=None)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(folder_path, f"{os.path.splitext(f)[0]}_Eigen_16bit_RGB.png"), dpi=300)
    plt.close()

print("\n✅ 16-bit RGB analysis complete.")
